...
//Email password based data show 

//but  Google login user data not show in this page not change my code add the feature 
// i want both feature 
...
const Myplant = () => {
  ...
  const fetchPlants = () => {
    if (user?.email) {
      console.log("Fetching plants for:", user.email);
      fetch(`https://treeplantcare-serversite.vercel.app/myplants?email=${user.email}`)
  ...
  useEffect(() => {
    ...
    const tryFetch = () => {
      if (user?.email) {
        ...
        attempts++;
        setTimeout(tryFetch, 300); // Retry after 300ms
      } else {
        console.warn("Email not found after retries.");
        setLoading(false);
      }
    };
        ...
        fetch(`https://treeplantcare-serversite.vercel.app/plants/${id}`, {
          ...
          .then(data => {
            if (data.deletedCount > 0) {
              ...
              Swal.fire('Failed!', 'Could not delete the plant.', 'error');
            }
          })
          .catch(error => {
